...
- (void)createLabel {
    ...
    if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_6_1) {
        labelSize = [self.data.labelText sizeWithFont:self.data.labelFont forWidth:self.data.labelWidth ?: 150 lineBreakMode:NSLineBreakByWordWrapping];
        
    } else {
        CGRect labelRect = [self.data.labelText boundingRectWithSize:CGSizeMake(self.data.labelWidth ?: 150, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.data.labelFont} context:nil];

        labelSize = CGSizeMake(labelRect.size.width, labelRect.size.height);
        
    }

    self.lblLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.radioButton.frame.origin.x + self.radioButton.frame.size.width + 15, 0, labelSize.width, labelSize.height)];
	[self updateLabel];
